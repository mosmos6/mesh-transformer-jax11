...
class TransformerLayerShard(nn.Module):
    ...
    def qvk_proj(self, x):
        ...
        q = self.q(x).reshape(x.shape[0] + (self.heads_per_shard, self.dim_per_head))
        v = self.v(x).reshape(x.shape[0] + (self.heads_per_shard, self.dim_per_head))
        k = self.k(x).reshape(x.shape[0] + (self.heads_per_shard, self.dim_per_head))
